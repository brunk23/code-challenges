...
int longest_distance(const int possible[CITIES], const int full[CITIES][CITIES]);
...
int main()
{
	...
	//		cout << "Checking " << names[x] << endl;
			temp = longest_distance(distance[x],distance);
			if ( temp > longest ) {
				longest = temp;
			}
		}
	...
	cout << "The longest distance was " << longest << endl;

	return 0;
}

/*
 * Will recursively call itself to find the longest distances
 */
int longest_distance(const int possible[CITIES],
			const int full[CITIES][CITIES])
{
	...
	// a distance > 0, and find the longest.
	for(int i = 0; i < CITIES; ++i) {
		if( possible[i] > 0 ) {
			...
			remaining += longest_distance(left, full);			
			if( remaining > longest ) {
				longest = remaining;
			}
		}
	}
	// cout << "returning: " << longest << endl;
	return longest;
}
